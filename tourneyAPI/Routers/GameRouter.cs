    ...
    public static void Map(WebApplication app, ILogger logger)
    {
        ...
        GameRoutes.MapGet("/", async (HttpContext context, AppDBContext db) =>
        {
            ...
            try
            {
                ...
                logger.LogInformation("Games Retrieved Successfully \n Time: {Timestamp}", DateTime.UtcNow);
                await context.Response.WriteAsJsonAsync(Games);
            }
            catch (Exception e)
            {
                context.Response.StatusCode = StatusCodes.Status500InternalServerError;
                logger.LogError("Unknown Error Occurred: {e} Time: {Timestamp}", e.ToString(), DateTime.UtcNow);
        ...
        GameRoutes.MapPost("/", async (HttpContext context, AppDBContext db) =>
        {
            ...
            try
            {
                ...
                if (newGame is null)
                {
                    ...
                    logger.LogWarning("Warning: newGame is null\n Time: {Timestamp}", DateTime.UtcNow);
                ...
                else
                {
                    ...
                    logger.LogInformation("Game Created Successfully. \n Time: {Timestamp}", DateTime.UtcNow);
        ...
        GameRoutes.MapPut("/", async (HttpContext context, AppDBContext db) =>
        {
            ...
            try
            {
                ...
                {
                    context.Response.StatusCode = StatusCodes.Status400BadRequest;
                    logger.LogWarning("Warning: updateGame is null\n Time: {Timestamp}", DateTime.UtcNow);
                    return Results.BadRequest("Invalid JSON Payload");
                }
                else
                {
                    ...
                    logger.LogInformation("Game Updated Successfully. \n Time: {Timestamp}", DateTime.UtcNow);
        ...
        GameRoutes.MapDelete("/", async (HttpContext context, AppDBContext db) =>
        {
            ...
            try
            {
                ...
                {
                    context.Response.StatusCode = StatusCodes.Status400BadRequest;
                    logger.LogWarning("Warning: deleteGame is null\n Time: {Timestamp}", DateTime.UtcNow);
                    return Results.BadRequest("Invalid JSON Payload");
                }
                else
                {
