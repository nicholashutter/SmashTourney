...
// Every expected path now carries the game id, because the destination screens
// live at game-scoped URLs. A redirect that dropped the id would land a player
// on a page that no longer knows which tournament it is showing.
...
{
    const redirect = resolveInMatchRedirect(null, activeMatch, "player-1", GAME_ID);
...
{
    const redirect = resolveInMatchRedirect("LOBBY_WAITING", activeMatch, "player-1", GAME_ID);
    expect(redirect).toBe("/lobby/game-id");
});
...
{
    const redirect = resolveInMatchRedirect("BRACKET_VIEW", activeMatch, "player-1", GAME_ID);
    expect(redirect).toBe("/showBracket/game-id");
});

// Verifies redirect to bracket when game is complete.
test("resolveInMatchRedirect routes to showBracket for COMPLETE", () =>
{
    const redirect = resolveInMatchRedirect("COMPLETE", activeMatch, "player-1", GAME_ID);
    expect(redirect).toBe("/showBracket/game-id");
});
...
{
    const redirect = resolveInMatchRedirect("IN_MATCH_ACTIVE", null, "player-1", GAME_ID);
    expect(redirect).toBe("/showBracket/game-id");
});
...
{
    const redirect = resolveInMatchRedirect("IN_MATCH_ACTIVE", activeMatch, "player-3", GAME_ID);
    expect(redirect).toBe("/showBracket/game-id");
});
...
{
    const redirect = resolveInMatchRedirect("IN_MATCH_ACTIVE", activeMatch, "player-1", GAME_ID);
    expect(redirect).toBeNull();
});
